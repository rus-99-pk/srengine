...
type AgentConfig struct {
	...
	PromptPath      string
	SummarizeEvery  int // compress context every N steps (0 = disabled)
}
...
type EmailConfig struct {
	...
	SMTPPort int
	From     string
	To       []string // multiple recipients supported
	Password string
}
...
type WebhookConfig struct {
	Enabled bool
	URL     string
	Headers map[string]string // optional custom headers, e.g. Authorization
	Timeout time.Duration
}
...
func Load() (*Config, error) {
	...
	// Namespaces — comma-separated list of monitored namespaces
...
// validate checks that required fields are present when a feature is enabled.
