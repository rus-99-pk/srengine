...
// promSeries — одна временна́я серия из Prometheus
...
// formatMetrics форматирует серии в человекочитаемый вид.
// Если переданы limitSeries — вычисляет утилизацию с severity-меткой.
func formatMetrics(usageSeries, limitSeries []promSeries, rangeMin int, isMemory bool) string {
	...
	// Лимит — last value из первой серии
	limitBytes := 0.0
...
// compactLabel оставляет только значимые лейблы — убирает технический мусор cadvisor
