...
// ScenarioResult represents the investigation output from the notifier log.
...
// Action represents a recommended step to resolve the issue.
type Action struct {
...
// Scenario describes a single integration test case.
...
// scenarios defines the list of all integration tests.
var scenarios = []Scenario{
	...
	},
	{
		...
		WantActionsContain:    []string{"memory", "limit"},
		WantMaxSteps:          8,
		// Wait longer so Prometheus can scrape metrics (3+ cycles of 15s).
		WaitForCrash: 90 * time.Second,
	},
}

// TestScenarios runs all defined integration test cases.
func TestScenarios(t *testing.T) {
	...
	for _, sc := range scenarios {
		...
		t.Run(sc.Name, func(t *testing.T) {
			// Run sequentially because the agent processes alerts one by one.
			runScenario(t, sc, webhookURL, resultDir)
		})
	}
}

// runScenario sets up and executes a single test scenario.
func runScenario(t *testing.T, sc Scenario, webhookURL, resultDir string) {
	...
	// Wait for pods to enter the desired crash/failure state.
	t.Logf("waiting %s for pods to crash...", sc.WaitForCrash)
	time.Sleep(sc.WaitForCrash)

	// Resolve the actual pod name if only a prefix is provided in labels.
	...
	// Send the alert payload to the agent.
	result := sendAlertAndWait(t, webhookURL, sc)

	// Save the investigation result to disk.
	saveResult(t, resultDir, sc.Name, result)

	// Assert test expectations against the result.
	assertResult(t, sc, result)
}

// sendAlertAndWait sends the alert payload and blocks until the investigation completes.
func sendAlertAndWait(t *testing.T, webhookURL string, sc Scenario) *ScenarioResult {
	...
	// Wait for the asynchronous investigation to complete using the configured timeout.
...
// waitForResult polls the /result endpoint until a final response is received.
func waitForResult(t *testing.T, baseURL, fingerprint string, timeout time.Duration) (*ScenarioResult, error) {
	...
	for time.Now().Before(deadline) {
		...
		if resp.StatusCode == http.StatusAccepted {
			// 202 = investigation is still in progress.
			resp.Body.Close()
...
// assertResult verifies the investigation result matches expectations.
func assertResult(t *testing.T, sc Scenario, result *ScenarioResult) {
	...
	// Root cause must contain at least one of the expected keywords.
	...
	// Confidence must be one of the acceptable values.
	...
	// Actions must mention at least one of the expected keywords.
	...
	// Step count must not exceed the defined limit.
...
// applyManifest applies the given Kubernetes manifest file.
...
// deleteNamespace securely removes a test namespace.
...
// findPodByPrefix returns the actual pod name given a deployment prefix.
...
// buildAlertPayload constructs a mock Alertmanager webhook JSON payload.
...
// saveResult writes the investigation result JSON to the configured directory.
...
// resolveManifestPath converts a relative path to an absolute path.
...
// getEnv retrieves a string environment variable or a fallback default.
...
// getEnvDuration retrieves a duration environment variable or a fallback default.
...
// cordonAndDrainNode prepares a node for testing NotReady alerts by simulating kubelet failure.
func cordonAndDrainNode(t *testing.T, nodeName string) {
	...
	// Pause the docker container to stop the kubelet heartbeat.
...
// waitForNodeReady blocks until a given node returns to the Ready state.
...
// getWorkerNode retrieves the first available non-control-plane node name.
